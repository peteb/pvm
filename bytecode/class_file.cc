...
std::string bytecode::class_file::str() const {
  std::stringstream ss;
  ...
  for (const auto &method : methods) {
    ss << "  " << modifiers_str(method.access_flags, false) + " " + describe_utf8(method.name_idx) << "\n";
    ss << "    descriptor: " << describe_utf8(method.descriptor_idx) << "\n";
    ss << "    flags: " << access_flags_str(method.access_flags) << "\n";

    for (const auto &attr : method.attributes) {
      ss << describe_utf8(attr->name_idx) << "\n";
    }
  }
...
std::string bytecode::class_file::describe_attributes(const std::vector<attribute_info_t *> attributes, int indent) const {
  std::stringstream ss;

  for (auto *attribute : attributes) {
    std::string name = describe_utf8(attribute->name_idx);
    ss << name + ": ";
    if (name == "SourceFile") {
      uint16_t source_idx = f2h(reinterpret_cast<attribute_source_file_t *>(attribute)->sourcefile_idx);
      ss << describe_utf8(source_idx);
    }
    ss << "\n";
  }
...
std::string bytecode::class_file::describe_utf8(uint16_t idx) const {
  auto utf8 = constant<cp_utf8_info_t>(idx);
  return std::string(utf8->bytes, utf8->length);
}

std::string bytecode::class_file::describe_class(uint16_t idx) const {
  auto klass = constant<cp_class_info_t>(idx);
  return describe_utf8(klass->name_idx);
}

std::string bytecode::class_file::describe_name_and_type(uint16_t idx) const {
  auto name_and_type = constant<cp_name_and_type_info_t>(idx);
  return describe_utf8(name_and_type->name_idx) + ":" + describe_utf8(name_and_type->descriptor_idx);
}

std::string bytecode::class_file::describe(cp_info_t *info) const {
  std::stringstream ss;
  ...
  case cp_tag::UTF8: {
    auto *utf8 = reinterpret_cast<cp_utf8_info_t *>(info);
    ss << "Utf8";
  ...
  case cp_tag::METHODREF: {
    auto *methodref = reinterpret_cast<cp_methodref_info_t *>(info);
    ss << "Methodref";
  ...
  case cp_tag::CLASS: {
    auto *class_info = reinterpret_cast<cp_class_info_t *>(info);
    ss << "Class";
    ss << std::setw(15) << std::left;
  ...
  case cp_tag::NAME_AND_TYPE: {
    auto *name_and_type = reinterpret_cast<cp_name_and_type_info_t *>(info);
    ss << "NameAndType";
    ss << std::setw(15) << std::left;
    ss << "#" + lexical_cast(name_and_type->name_idx) + ":" + "#" + lexical_cast(name_and_type->descriptor_idx);
    ss << "// " + describe_utf8(name_and_type->name_idx) + ":" + describe_utf8(name_and_type->descriptor_idx);
    break;
  }

  case cp_tag::FIELDREF: {
    auto *fieldref = reinterpret_cast<cp_fieldref_info_t *>(info);
    ss << "Fieldref";
    ss << std::setw(15) << std::left;
  ...
  case cp_tag::STRING: {
    auto *string_info = reinterpret_cast<cp_string_info_t *>(info);
    ss << "String";
    ss << std::setw(15) << std::left;
    ss << "#" + lexical_cast(string_info->string_idx);
    ss << "// " + describe_utf8(string_info->string_idx);
    break;
  }
